...
export default function StudentProfile() {
  ...
  const onSaveProfile = async (values) => {
    try {
      setSaving(true)
      const { data } = await profileApi.updateStudentProfile(values)
      setProfile(data)
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      ...
      const { data } = await profileApi.uploadStudentAvatar(formData)
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-4 flex items-center gap-4">
        <div className="relative">
          ...
          <button
            ...
            disabled={uploading}
            className="absolute bottom-0 right-0 w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center text-xs hover:bg-blue-700"
          >
            {uploading ? '...' : '✏️'}
          </button>
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">{profile?.programName} — {profile?.departmentName}</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <form onSubmit={handleSubmit(onSaveProfile)} className="space-y-4">
            ...
            <div>
              ...
              <input {...register('address')} className="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
